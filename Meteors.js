...
export default class MeteorScreen extends Component {
  ...
  getMeteors = () => {
    axios
      .get(
        'https://api.nasa.gov/neo/rest/v1/feed?api_key=ScWn8YE74cbAaBJWW34khM1uMgmONQqQ0cvllUtK'
      )
      .then((response) => {
  ...
  render() {
    if (Object.keys(this.state.meteors).length === 0) {
      ...
      meteors.forEach(function (element) {
        let diameter =
          (element.estimated_diameter.miles.estimated_diameter_min +
            element.estimated_diameter.miles.estimated_diameter_max) /
          2;
        let threatScore =
          (diameter / element.close_approach_data[0].miss_distance.miles) *
          1000000000;
